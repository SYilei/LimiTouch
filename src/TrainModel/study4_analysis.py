...
path = '../../data/Study4_derivative/'

data_files = os.listdir(path)
column = ['0','1','2','3','4','5','6','7','8'] 
gestures = ['chair','handback','singlehand','sofa','thigh','wall','window','wood']
accuracy = [[] for i in range(8)]
print(accuracy)
for index in range(len(gestures)):
    for file in data_files:
        if '.csv' in file and gestures[index] in file:
            data = pd.read_csv(path + file)[column].values
            correct = 0
            total = 0
            for i in range(0, len(data) - 250, 50):
                ...
                if y_pred == 1:
                    correct += 1
            print(round(correct / total, 4), file)
            accuracy[index].append(correct / total)    

for i in range(8):
    print(gestures[i],accuracy[i])
