...
def on_press(key):
    ...
    if eventkey == 'Key.esc':
        record = open('../../data/Data_Study3/record_'+sys.argv[1]+'.txt','w+')
...
def generate_gesture_order():
    ...
    list_gesture = ['microwave', 'small bottle', 'large bottle', 'door handle', 'cabinet', 'door']
    while len(list_gesture) > 0:
        gesture = list_gesture.pop(random.randint(0, len(list_gesture) - 1))
        list_touch = ['touch', 'nontouch']
        while len(list_touch) > 0:
            touch = list_touch.pop(0)
            gesture_order.append((touch+'_'+gesture))
    final_gesture_list = []
...
if len(sys.argv) == 2:
    ...
    file = open('../../data/Data_Study3/record_' + sys.argv[1]+'.txt', 'r')
    line = file.readline()
...
while loop:
    ...
    if count == 30000:
        write_to_csv('../../data/Data_Study3/' + id_user + '_' + gestures[0] + '.csv', data_list)
        read_data = False
        data_list.clear()
        count = 0
        gestures.pop(0)
        if len(gestures) > 0:
            print('')
            print('-----------------------------')
            print('Gesture'+ str(12 - len(gestures))+': ' + gestures[0])
        else:
            break
